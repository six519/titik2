...
func (parser Parser) Parse(tokenArray []Token, globalVariableArray *[]Variable, globalFunctionArray *[]Function, scopeName string, globalNativeVarList *[]string, gotReturn *bool, returnToken *Token, isLoop bool, needBreak *bool, stackReference *[]Token, globalSettings *GlobalSettingsObject) error {
					...
					for len(outputQueue) > 0 {
						...
						if(currentToken.Type == TOKEN_TYPE_PLUS || currentToken.Type == TOKEN_TYPE_MINUS || currentToken.Type == TOKEN_TYPE_DIVIDE || currentToken.Type == TOKEN_TYPE_MULTIPLY) {
							...
							if((*globalFunctionArray)[funcIndex].IsNative) {
								...
								prsr := Parser{}
								parserErr := prsr.Parse((*globalFunctionArray)[funcIndex].Tokens, globalVariableArray, globalFunctionArray, thisScopeName, globalNativeVarList, &thisGotReturn, &thisReturnToken, false, &thisNeedBreak, &thisStackReference, globalSettings)
							...
							*needBreak = true
								...
								if(currentToken.Type == TOKEN_TYPE_IF_END || currentToken.Type == TOKEN_TYPE_ELSE || currentToken.Type == TOKEN_TYPE_ELIF_START) {
									if(openIfCount == 0) {
										if(currentToken.Type == TOKEN_TYPE_IF_END) {
											...
											prsr := Parser{}
											parserErr := prsr.Parse(paramTokens, globalVariableArray, globalFunctionArray, scopeName, globalNativeVarList, &efGotReturn, &efReturnToken, isLoop, &efNeedBreak, &efStackReference, globalSettings)
							...
							if(executeIfStatement) {
								...
								prsr := Parser{}
								parserErr := prsr.Parse(tempTokens[currentStatementIndex].Tokens, globalVariableArray, globalFunctionArray, scopeName, globalNativeVarList, &ifGotReturn, &ifReturnToken, isLoop, &ifNeedBreak, &ifStackReference, globalSettings)
