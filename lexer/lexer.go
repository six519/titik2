			...
			switch(tokenizerState) {
				case TOKENIZER_STATE_GET_WORD:
					...
					if(unicode.IsLetter([]rune(currentChar)[0]) || currentChar == "_") {
						...
						return finalTokenArray, errors.New(info.SyntaxErrorMessage(x + 1, x2 + 1, "Invalid token", lexer.FileName))
					}
	...
	for x := 0; x < len(tokenArray); x++ {
		if(tokenArray[x].Type == TOKEN_TYPE_IDENTIFIER) {
			...
			if(!((x+1) < len(tokenArray))) {
				return finalTokenArray, errors.New(info.SyntaxErrorMessage(tokenArray[x].Line, tokenArray[x].Column, "Expected closing of string", lexer.FileName))
			}
		} else if(tokenArray[x].Type == TOKEN_TYPE_MULTI_COMMENT) {
			if(!((x+1) < len(tokenArray))) {
				return finalTokenArray, errors.New(info.SyntaxErrorMessage(tokenArray[x].Line, tokenArray[x].Column, "Expected closing of multi line comment", lexer.FileName))
			}
		}
